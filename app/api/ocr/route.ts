...
요구사항:
1. 대화창 상단에 표시된 실제 이름을 파싱해서 사용해주세요
   - 내 메시지는 "나:" 형식으로 표시
   - 상대방 메시지는 대화창 상단에 표시된 실제 이름을 사용 (예: "이름:" 형식)
2. 이모티콘은 그대로 유지해주세요
3. 시간 정보는 제외해주세요
4. 순서대로 정확하게 추출해주세요
5. 한국어 대화만 추출해주세요`;
...
위 맥락을 참고하여, 이 이미지의 대화도 동일한 이름 형식을 사용하여 추출해주세요.
대화창 상단의 실제 이름을 파싱해서 사용하고, 각 메시지가 누구의 말인지 정확히 구분하여 접두사를 붙여주세요.`;
    }
